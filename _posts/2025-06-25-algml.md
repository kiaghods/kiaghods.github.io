...
Special thanks are also due to [Nick James](https://www.linkedin.com/in/nicholas-james-25520027b/), whose critical feedback prompted a significant revision of the introductory sections. His push for more geometric intuition and visual examples, specifically referencing the dihedral group and the value of visual group theory, greatly improved the final result.
...
#### Footnotes
[^1]: Observe that commutativity is not one of the group axioms; indeed, not all groups are commutative. Groups whose operation is commutative are referred to as _abelian groups_. For example, the set of pure rotations within $D_4$, which is $\lbrace e, r, r^2, r^3 \rbrace$, forms an abelian subgroup, since the order of rotations does not change the outcome (e.g., $r \circ r^2 = r^2 \circ r$).
